classdef LOD_calculations_beta_1 < matlab.apps.AppBase
    ...
    methods (Access = private)
        ...
        function FitButtonPushed(app, event)
            ...
            %Adjust concentration to set order of magnitude for better
            %fitting
            app.factor=10^round(log10(min(testConc)));
            testConc=testConc/app.factor;
            ...
            app.fitresult = fitnlm(xData, yData, ft,b0);
            ...
            app.LOD_lower95 = app.LOD_lower95*app.factor;
            app.LOD_upper95 = app.LOD_upper95*app.factor;
            %-- Plot result --%
            ...
            app.allTestData_testConc=app.allTestData_testConc*app.factor;%adjust concentrations back to original magnitude
            ...
            fitData_x = logspace(log10(minpoint),log10(max(app.allTestData_testConc_logplus2)),50);
            ...
            fitData_x=fitData_x*app.factor;
            ...
            app.T2 = table(["LOD";"LOD lower";"LOD upper";"Confidence level negatives";"Confidence level positives";"Confidence level variances";"Fit parameters";"L_D";"L_C";"Conc factor";"LOD log";"LOD log SE";"Total reps";"Number of parameters";"RMSE";"AICc";"Units"],[string(app.LOD);app.LOD_lower95;app.LOD_upper95;alpha;beta;varAlpha;fitpars_string;app.Lc;app.Ld;app.factor;app.logplus2_LOD;app.logplus2_LOD_SE;app.numReps_Total;length(fitpars);app.fitresult.RMSE;app.fitresult.ModelCriterion.AICc;app.concUnits],'VariableNames',{'Parameter','Value'});
        end
        ...
        function SavematButtonPushed(app, event)
            ...
            if file==0
                ...
                factor=app.factor;
                save(fullfile(path,file),'xn','yn','stdn','allTestData_testConc','allTestData_pixInt','allData_pixInt','Lc','Ld','LOD','LOD_upper95','LOD_lower95','numReps_negCtrl','pixInt','allTestData_testConc_logplus2','fitresult','logplus2_LOD_SE','logplus2_LOD','numReps_Total','concUnits','factor');
                figure(app.LODcalculationsUIFigure)
            end
        end
        ...
        function PlotselectedfilesButtonPushed(app, event)
            ...
            for i=1:length(items_list)
                ...
                if M.LOD_lower95<min(M.allTestData_testConc) && M.LOD_lower95>0
                    ...
                    minpoint_temp=log10(M.LOD_lower95/M.factor+2);
    ...
    methods (Access = public)
        ...
        % Construct app
        function app = LOD_calculations_beta_1
